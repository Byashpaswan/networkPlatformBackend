...
async function assignWork() {
    let jobs = 0;
    try {
        let result = await PlatformTypeModel.getPlatformTypesOne({ name: 'Youmi' }, {});
        if (result) {
        ...
        console.log("Youmi Job Inserted ", jobs);

    }
    catch (err) {
        console.log("error occured during assigning jobs Youmi");
