...
const debug = require("debug")("darwin:Plugin:Mobimelon");
...
exports.callMobimelonApi = async () => {
    try {
...
async function assignWork() {
    let jobs = 0;
    try {
        let result = await PlatformTypeModel.getPlatformTypesOne({ name: 'Mobimelon/ Digiadsnet/ Admusketeer' }, {});
        if (result) {
        ...
        console.log("Mobimelon Job Inserted ", jobs);

    }
    catch (err) {
        console.log("error occured during assigning jobs Mobimelon");
