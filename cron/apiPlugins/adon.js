...
async function assignWork() {
    let jobs = 0;
    try {
        let result = await PlatformTypeModel.getPlatformTypesOne({ name: 'AdOn' }, {});
        if (result) {
        ...
        console.log("AdOn Job Inserted ", jobs);

    }
    catch (err) {
        console.log("error occured during assigning jobs AdOn");
