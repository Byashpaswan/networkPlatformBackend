...
async function assignWork() {
    let jobs = 0;
    try {
        let result = await PlatformTypeModel.getPlatformTypesOne({ name: 'Adbink' }, {});
        if (result) {
        ...
        console.log("Adbink Job Inserted ", jobs);

    }
    catch (err) {
        console.log("error occured during assigning jobs Adbink");
