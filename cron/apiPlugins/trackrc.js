...
async function assignWork() {
    let jobs = 0;
    try {
        let result = await PlatformTypeModel.getPlatformTypesOne({ name: 'TrackRC' }, {});
        if (result) {
        ...
        console.log("TrackRC Job Inserted ", jobs);

    }
    catch (err) {
        console.log("error occured during assigning jobs TrackRC");
