...
async function assignWork() {
    let jobs = 0;
    try {
        let result = await PlatformTypeModel.getPlatformTypesOne({ name: 'Avazu' }, {});
        if (result) {
        ...
        console.log("Avazu Job Inserted ", jobs);

    }
    catch (err) {
        console.log("error occured during assigning jobs Avazu");
