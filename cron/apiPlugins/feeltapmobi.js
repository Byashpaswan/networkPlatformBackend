...
const debug = require("debug")("proffcusdb:Plugin:Feeltapmobi");
...
exports.callFeeltapmobiApi = async () => {
    try {
...
async function assignWork() {
    let jobs = 0;
    try {
        let result = await PlatformTypeModel.getPlatformTypesOne({ name: 'Feeltapmobi' }, {});
        if (result) {
        ...
        console.log("Feeltapmobi Job Inserted ", jobs);

    }
    catch (err) {
        console.log("error occured during assigning jobs Feeltapmobi");
