...
const debug = require("debug")("darwin:apiPlugin:Newseed");
...
async function assignWork() {
    let jobs = 0;
    try {
        let result = await PlatformTypeModel.getPlatformTypesOne({ name: 'Newseed' }, {});
        if (result) {
        ...
        console.log("Newseed Job Inserted ", jobs);

    }
    catch (err) {
        console.log("error occured during assigning jobs Newseed");
