...
async function assignWork() {
    let jobs = 0;
    try {
        let result = await PlatformTypeModel.getPlatformTypesOne({ name: 'Gowide' }, {});
        if (result) {
        ...
        console.log("Gowide Job Inserted ", jobs);

    }
    catch (err) {
        console.log("error occured during assigning jobs Gowide");
