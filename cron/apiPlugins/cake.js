...
async function assignWork() {
    let jobs = 0;
    try {
        let result = await PlatformTypeModel.getPlatformTypesOne({ name: 'Cake' }, {});
        if (result) {
        ...
        console.log("Cake Job Inserted ", jobs);

    }
    catch (err) {
        console.log("error occured during assigning jobs Cake");
