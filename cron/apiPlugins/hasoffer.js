...
async function assignWork() {
    let jobs = 0;
    try {
        let result = await PlatformTypeModel.getPlatformTypesOne({ name: 'Hasoffer' }, {});
        if (result) {
        ...
        console.log("Hasoffer Job Inserted ", jobs);

    }
    catch (err) {
        console.log("error occured during assigning jobs Hasoffer");
