...
async function assignWork() {
    let jobs = 0;
    try {
        let result = await PlatformTypeModel.getPlatformTypesOne({ name: 'Appflood' }, {});
        if (result) {
        ...
        console.log("Appflood Job Inserted ", jobs);

    }
    catch (err) {
        console.log("error occured during assigning jobs Appflood");
