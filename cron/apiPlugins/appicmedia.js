...
const debug = require("debug")("darwin:Plugin:Appicmedia");
...
exports.callAppicmediaApi = async () => {
    try {
...
async function assignWork() {
    let jobs = 0;
    try {
        let result = await PlatformTypeModel.getPlatformTypesOne({ name: 'Appicmedia' }, {});
        if (result) {
        ...
        console.log("Appicmedia Job Inserted ", jobs);

    }
    catch (err) {
        console.log("error occured during assigning jobs Appicmedia");
