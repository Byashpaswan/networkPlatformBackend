...
async function assignWork() {
    let jobs = 0;
    try {
        let result = await PlatformTypeModel.getPlatformTypesOne({ name: 'Trackier' }, {});
        if (result) {
        ...
        console.log("Trackier Job Inserted ", jobs);

    }
    catch (err) {
        console.log("error occured during assigning jobs Trackier");
