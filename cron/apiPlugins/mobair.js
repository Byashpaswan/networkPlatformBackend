...
const debug = require("debug")("darwin:Plugin:Mobair");
...
async function assignWork() {
    let jobs = 0;
    try {
        let result = await PlatformTypeModel.getPlatformTypesOne({ name: 'Mobair' }, {});
        if (result) {
        ...
        console.log("Mobair Job Inserted ", jobs);

    }
    catch (err) {
        console.log("error occured during assigning jobs Mobair");
