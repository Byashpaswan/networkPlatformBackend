...
async function assignWork() {
    let jobs = 0;
    try {
        let result = await PlatformTypeModel.getPlatformTypesOne({ name: 'Vene' }, {});
        if (result) {
        ...
        console.log("Vene Job Inserted ", jobs);

    }
    catch (err) {
        console.log("error occured during assigning jobs Vene");
