...
async function assignWork() {
    let jobs = 0;
    try {
        let result = await PlatformTypeModel.getPlatformTypesOne({ name: 'Gazet' }, {});
        if (result) {
        ...
        console.log("Gazet Job Inserted ", jobs);

    }
    catch (err) {
        console.log("error occured during assigning jobs Gazet");
