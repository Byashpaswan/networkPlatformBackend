...
async function assignWork() {
    let jobs = 0;
    try {
        let result = await PlatformTypeModel.getPlatformTypesOne({ name: 'Cpology' }, {});
        if (result) {
        ...
        console.log("Cpology Job Inserted ", jobs);

    }
    catch (err) {
        console.log("error occured during assigning jobs Cpology");
