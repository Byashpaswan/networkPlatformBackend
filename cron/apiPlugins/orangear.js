...
async function assignWork() {
    let jobs = 0;
    try {
        let result = await PlatformTypeModel.getPlatformTypesOne({ name: 'Orangear (Tapgerine Type)' }, {});
        if (result) {
        ...
        console.log("Orangear Job Inserted ", jobs);

    }
    catch (err) {
        console.log("error occured during assigning jobs Orangear");
