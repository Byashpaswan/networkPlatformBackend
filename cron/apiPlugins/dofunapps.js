...
const debug = require("debug")("darwin:Plugin:DofunApps");
...
exports.callDofunAppsApi = async () => {
    try {
...
async function assignWork() {
    let jobs = 0;
    try {
        let result = await PlatformTypeModel.getPlatformTypesOne({ name: 'Dofunapps/ Mobpals' }, {});
        if (result) {
        ...
        console.log("DofunApps Job Inserted ", jobs);

    }
    catch (err) {
        console.log("error occured during assigning jobs DofunApps");
