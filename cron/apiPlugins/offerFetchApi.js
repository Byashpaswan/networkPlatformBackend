...
const { getAid, getNid, getPlid, getPlty } = require("../../helpers/Functions")
const extraQueue = { "Affise": "Affise_Api_queue", "Offerslook": "Offerslook_Api_queue" }
const generalFuntion = require('../../helpers/generalFunction')
...
async function assignWork() {
    ...
    try {
        let result = await PlatformTypeModel.getPlatformTypes({}, { name: 1 }, { sort: { _id: -1 } });
        if (result) {
            for (let i = 0; i < result.length; i++) {
                ...
                if (platforms) {
                    for (let m = 0; m < platforms.length; m++) {
                        let obj = platforms[m];
                        let advertiserData = await generalFuntion.getAdvertiser(obj.advertiser_id);
                        if(advertiserData && Object.keys(advertiserData).length > 0 && advertiserData['status'] != 'Active'){
                            continue;
                        }
                        if (obj['autoFetch']) {
                            let credentials = {};
                            ...
                            content['nid'] = obj.nid || await getNid(obj.network_id);
                            content['advertiser_id'] = obj.advertiser_id;
                            content['aid'] = obj.aid || await getAid(obj.advertiser_id);
                            content['advertiser_name'] = obj.advertiser_name;
                            content['platform_id'] = obj.platform_id;
                            content['plty'] = obj.plty || await getPlty(obj.platform_id);
                            ...
                            content['plid'] = obj.plid || await getPlid(obj._id);
        ...
        debug(err)
        console.log("error occured during assigning jobs Afftrack");
