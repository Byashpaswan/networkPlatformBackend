...
async function assignWork() {
    let jobs = 0;
    try {
        let result = await PlatformTypeModel.getPlatformTypesOne({ name: 'Taptica' }, {});
        if (result) {
        ...
        console.log("Taptica Job Inserted ", jobs);

    }
    catch (err) {
        console.log("error occured during assigning jobs Taptica");
