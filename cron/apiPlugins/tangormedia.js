...
const debug = require("debug")("darwin:Plugin:Tangormedia");
...
exports.callTangormediaApi = async () => {
    try {
...
async function assignWork() {
    let jobs = 0;
    try {
        let result = await PlatformTypeModel.getPlatformTypesOne({ name: 'Tangormedia' }, {});
        if (result) {
        ...
        console.log("Tangormedia Job Inserted ", jobs);

    }
    catch (err) {
        console.log("error occured during assigning jobs Tangormedia");
