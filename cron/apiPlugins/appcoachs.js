...
const debug = require("debug")("darwin:Plugin:Appcoach");
...
exports.callAppcoachsApi = async () => {
    try {
...
async function assignWork() {
    let jobs = 0;
    try {
        let result = await PlatformTypeModel.getPlatformTypesOne({ name: 'Appcoach' }, {});
        if (result) {
        ...
        console.log("Appcoach Job Inserted ", jobs);

    }
    catch (err) {
        console.log("error occured during assigning jobs Appcoach");
