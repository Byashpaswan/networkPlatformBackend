...
async function assignWork() {
    let jobs = 0;
    try {
        let result = await PlatformTypeModel.getPlatformTypesOne({ name: 'Trakneo' }, {});
        if (result) {
        ...
        console.log("Trakneo Job Inserted ", jobs);

    }
    catch (err) {
        console.log("error occured during assigning jobs Trakneo");
