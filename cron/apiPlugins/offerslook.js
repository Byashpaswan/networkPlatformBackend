...
var publish_queue = 'Offerslook_Api_queue';
...
exports.callOfferslookApi = async () => {
    try {
...
async function assignWork() {
    let jobs = 0;
    try {
        let result = await PlatformTypeModel.getPlatformTypesOne({ name: 'Offerslook' }, {});
        if (result) {
        ...
        console.log("Offerslook Job Inserted ", jobs);

    }
    catch (err) {
        console.log("error occured during assigning jobs Offerslook");
