...
async function assignWork() {
    let jobs = 0;
    try {
        let result = await PlatformTypeModel.getPlatformTypesOne({ name: 'Auctera' }, {});
        if (result) {
        ...
        console.log("Auctera Job Inserted ", jobs);

    }
    catch (err) {
        console.log("error occured during assigning jobs Auctera");
