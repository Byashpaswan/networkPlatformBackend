...
var publish_queue = 'Affise_Api_queue';
...
async function assignWork() {
    let jobs = 0;
    try {
        let result = await PlatformTypeModel.getPlatformTypesOne({ name: 'Affise' }, {});
        if (result) {
        ...
        console.log("Affise Job Inserted ", jobs);

    }
    catch (err) {
        console.log("error occured during assigning jobs Affise");
