...
const debug = require("debug")("darwin:Plugin:Adon");
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.api_key && credentials.network_id) {
        ...
        let apiBaseurl = "https://" + network_id + "/api/v2.0/getOffers?api_key=" + api_key + "&limit=" + apilimit + "&page=";
...
exports.checkValid = (result) => {
    if (result.errcode == 0 && result.offers) {
        return true;
    }
    else {
        return false;
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = Adon[action](field, offer);
