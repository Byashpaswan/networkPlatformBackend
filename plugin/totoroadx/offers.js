...
const debug = require("debug")("darwin:Plugin:totoroadx");
...
exports.apiCall = async (credentials, page, apilimit) => {
  if (credentials.token) {
    ...
    let apiBaseurl = "http://" + network_id + "/offers/?id=" + id + '&token=' + token
    ...
    try {
      ...
      if (result) {
        ...
        if (valid === true) {
          ...
          content['domain'] = "api.totoroadx.com";
...
exports.checkValid = (result) => {
  if (result && result.counts !== 0 && result.offers && result.offers.length) {
    return true;
  } else {
    return false;
  }
...
function getIsTargeting(field, data) {
  ...
  if (data.country || data.os || data.os_version) {
...
function formateOffers(offer) {
  ...
  array.map(function (obj) {
    try {
      ...
      let data = totoroadx[action](field, offer);
