...
const debug = require("debug")("darwin:Plugin:retail9");
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.affiliate_id && credentials.network_id) {
        let affiliate_id = credentials['affiliate_id'];
        let network_id = credentials['network_id'];
        let apiBaseurl = "http://" + network_id + "/genxml.php?affid=" + affiliate_id + "&img=1&items=" + apilimit + "&page=";
        // debug(apiBaseurl)
...
exports.checkValid = (result) => {
    if ((result.count || result.count === 0) && result.offers) {
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = retail9[action](field, offer);
