...
const debug = require("debug")("darwin:Plugin:Octava");
...
exports.apiCall = async (credentials, page, apilimit) => {
  if (credentials.network_id && credentials.api_key) {
    let apiBaseurl = `https://${credentials.network_id}/api/v2.0/getOffers?api_key=${credentials.api_key}&page=${page}&limit=${apilimit}`
...
function getTrackingLink(field, data) {
  return data.click_url ? data.click_url : "";
}
...
function getApprovalRequired(field, data) {
  return data.click_url ? false : true;
}
...
function getIsTargeting(field, data) {
  return data.platform || data.country ? true : false;
}
...
function getDeviceTargeting(field, data) {
  ...
  if (data.platform) {
    if (data.platform.toLowerCase() == 'android' && !device_targeting.os.includes('android')) {
      device_targeting.os.push('android');
    } else if (data.platform.toLowerCase() == 'ios' && !device_targeting.os.includes('ios')) {
...
function formateOffers(offer) {
  ...
  array.map(function (obj) {
    try {
      let data = octava[obj.action](obj.field, offer);
