...
const debug = require("debug")("darwin:Plugin:miskyAds");
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.api_key && credentials.network_id) {
        ...
        let apiBaseurl = "https://" + network_id + "/?version=1&apikey=" + api_key + "&c=api.get_offers&limit=" + apilimit + "&my_offers=1&page=";
        // debug(apiBaseurl)
...
exports.offersApiCall = async (content) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            await this.miskyAdsOffersApiCall(content, offerLog, start_time);
            return resolve(true);
        }
        catch (err) {
            debug(err);
...
exports.miskyAdsOffersApiCall = async (content, offerLog, start_time) => {
    // debug('miskyAds');
    let api_key = content.credentials['api_key'];
    let network_id = content.credentials['network_id'];
    let apiBaseurl = "https://" + network_id + "/?version=1&apikey=" + api_key + "&c=api.get_offers&limit=" + limit + "&my_offers=0&page";
    let page = 1;
    let totalPages = 1;
    let valid = false;
    return new Promise(async (resolve, reject) => {
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = miskyAds[action](field, offer);
