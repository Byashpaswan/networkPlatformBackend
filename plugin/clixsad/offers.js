...
const debug = require("debug")("darwin:Plugin:clixsad");
...
exports.getPid = (url) =>{
    ...
    return urlData.searchParams.get('zoneId');
}
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.zoneid) {
        let zoneid = credentials['zoneid'];
        let apiBaseurl = `http://clixsad.adserverexchange.com/offerdetails?zoneid=` + zoneid;
            ...
            do {
                ...
                if (result) {
                    ...
                    if (valid === true) {
                        ...
                        content['domain'] = "clixsad.adserverexchange.com";
...
exports.checkValid = (result) => {
    // console.log(result,'------------------------');
    if (result && result.isValidToken == true && result.success == true && result.data && result.data.length) {
        return true;
    }
}
...
function getRevenue(field, data) {
    let revenue = 0.0;
    if (data.payableevent && data.payableevent.length) {
        data.payableevent.map(obj => {
            revenue = obj.payout
        })
    }
    return revenue;
...
function getRevenueType(field, data) {
    ...
    if (data.pricingmodel) {
        revenue_type.offer_type = data.pricingmodel.toLowerCase();
    }
    return revenue_type;
}
...
function getPayoutType(field, data) {
    ...
    if (data.pricingmodel) {
        payout_type.offer_type = data.pricingmodel.toLowerCase();
    }
    return payout_type;
}
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = clixsad[action](field, offer);
