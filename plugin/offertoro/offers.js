...
const debug = require("debug")("darwin:Plugin:offertoro");
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.pubid && credentials.appid && credentials.secretkey) {
        let pubid = credentials['pubid'];
        let appid = credentials['appid'];
        let secretkey = credentials['secretkey'];
        let apiBaseurl = "https://www.offertoro.com/api/?pubid=" + pubid + "&appid=" + appid + "&secretkey=" + secretkey + "&platform=all&format=json";
...
exports.checkValid = (result) => {
    if (result.response && result.response.offers.length) {
        return true;
    }
    return false;
}
...
function getTrackingLink(field, data) {
    let tracking_link = '';
    if (data.offer_url_easy) {
        tracking_link = data.offer_url_easy;
    }
...
function getIsTargeting(field, data) {
    ...
    if ((data.countries && data.countries.length) || (data.platform) || (data.device)) {
...
function getDeviceTargeting(field, data) {
    ...
    if (data.device && ['android', 'ios', 'windows', 'amazon', 'blackberry', 'unknown'].includes(data.device.toLowerCase())) {
        device_targeting.os.push(data.device.toLowerCase());
    }
    if (data.platform && ['mobile', 'desktop', 'unknown'].includes(data.platform.toLowerCase())) {
        device_targeting.device.push(data.platform.toLowerCase());
    }
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = offertoro[action](field, offer);
