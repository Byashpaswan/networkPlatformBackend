...
const debug = require("debug")("darwin:Plugin:attrimob");
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.api_key && credentials.network_id) {
        ...
        let network_id = credentials['network_id'];
         let apiBaseurl = "https://" + network_id + "/perf/v1/campaign/ext?apiKey="+ api_key;
        // debug(apiBaseurl)
...
exports.getPid = (url) =>{
    ...
    return urlData.searchParams.get('publisher_id');
}
...
function getTrackingLink(field, data) {
    let tracking_link = '';
    if (data.click_tracking_url)
        tracking_link = data.click_tracking_url;
...
function getStartDate(field, data) {
    ...
    try {
        if (data.campaign_from) {
            start_date = moment.unix(data.campaign_from)
...
function getEndDate(field, data) {
    ...
    try {
        if (data.campaign_to) {
            end_date = moment.unix(data.campaign_to);
...
function getRevenueType(field, data) {
    let revenue_type = { enum_type: 'unknown', offer_type: '' };
     if(data.pay_m){
        revenue_type.offer_type=data.pay_m;
     }
...
function getPayoutType(field, data) {
    ...
    if(data.pay_m){
        payout_type.offer_type=data.pay_m;
    }
...
function getGeoTargeting(field, data) {
    ...
    if(data.geo_targeting){
        geo_targeting.country_allow.push({key:data.geo_targeting,value:data.geo_targeting})
    }
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = Attrimob[action](field, offer);
