...
const debug = require("debug")("darwin:Plugin:trackrc");
...
exports.getPid = (url) =>{
    ...
    return urlData.searchParams.get('A1');
}
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.api_key) {
        ...
        let apiBaseurl = "http://www.trackrc.com/api/v1/offers?api_key=" + api_key + "&page_size=" + apilimit + "&page=";
        // debug(apiBaseurl)
            ...
            do {
                ...
                if (result) {
                    ...
                    if (valid === true) {
                        ...
                        content['domain'] = "www.trackrc.com";
...
exports.checkValid = (result) => {
    if (result.offers && result.err == 0) {
        //valid credentials trackrc  
        return true;
    }
    else {
        return false;
...
function getIsTargeting(field, data) {
    ...
    if ((data.platform && data.platform.length) || (data.countries && data.countries.length)) {
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = trackrc[action](field, offer);
