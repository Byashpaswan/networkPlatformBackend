...
const debug = require("debug")("darwin:Plugin:Vene");
...
exports.countApiPages = (response) => {
    let page = 0;
    try {
        if (response.meta && response.meta && response.meta.total) {
            page = Math.ceil(response.meta.total / limit);
        }
        // debug(page)
        return page;
    }
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.api_key && credentials.network_id) {
        ...
        let apiBaseurl = "https://" + network_id + "/api/publisher-api/v1/live-offers?limit=" + apilimit + "&offset=";
        // debug(apiBaseurl)
...
function getCurrency(field, data) {
    let currency = 'USD';
    if (data.payouts && data.payouts.length && data.payouts[0].currency) {
        currency = data.payouts[0].currency.toUpperCase();
    }
    return currency;
}
...
function getRevenue(field, data) {
    let revenue = 0.0;
    if (data.payouts && data.payouts.length && data.payouts[0].amount)
        revenue = data.payouts[0].amount;
    return revenue;
}

function getRevenueType(field, data) {
    let revenue_type = { enum_type: 'unknown', offer_type: '' };
    if (data.payouts && data.payouts.length && data.payouts[0].type)
        revenue_type.offer_type = data.payouts[0].type;
    return revenue_type;
}
...
function getPayoutType(field, data) {
    ...
    if (data.payouts && data.payouts.length && data.payouts[0].type)
        payout_type.offer_type = data.payouts[0].type;
    return payout_type;
}
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = Vene[action](field, offer);
