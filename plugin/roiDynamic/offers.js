...
const debug = require("debug")("darwin:Plugin:Roidynamic");
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.api_key && credentials.network_id) {
        ...
        let apiBaseurl = "https://" + network_id + "/publisher-api/offers?token=" + api_key;
        // debug(apiBaseurl)
...
function getIsCapEnabled(field, data) {
    ...
    if (data.capacity || data.dailyBudget) {
        cap_enable = true;
    }
    return cap_enable;
}
...
function getOfferCapping(field, data) {
    let cap = defaultCap();
    if (data.dailyBudget) {
        cap.daily_revenue = data.dailyBudget;
    }
    if (data.capacity) {
        cap.daily_conv = data.capacity;
    }
    return cap;
}
...
function getIsTargeting(field, data) {
    ...
    if ((data.platforms && data.platforms.length) || (data.geo && data.geo.length)) {
...
function getDeviceTargeting(field, data) {
    ...
    if (data.platforms && data.platforms.length) {
        data.platforms.map(obj => {
            ...
            if (os && data.minosVersion) {
                device_targeting.os_version.push({ os: os, version: data.minosVersion, version_condition: 'gte' });
            }
        });
    }
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = Roidynamic[action](field, offer);
