...
const debug = require("debug")("darwin:Plugin:appSamurai");
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.api_key) {
        ...
        let apiBaseurl = "http://api.appsamurai.com/api/campaign-pull/list/" + api_key;
            ...
            do {
                ...
                if (result) {
                    ...
                    if (valid === true) {
                        ...
                        content['domain'] = 'api.appsamurai.com';
...
function getEndDate(field, data) {
    ...
    // try {
    //     if (data['end_date'] && moment(data['end_date']).isValid()) {
...
function getRevenueType(field, data) {
    ...
    if (data['payout_type']) {
        revenue_type['offer_type'] = data['payout_type'].toUpperCase()
    }
    return revenue_type;
}
...
function getPayoutType(field, data) {
    ...
    if (data['payout_type']) {
        payout_type['offer_type'] = data['payout_type'].toUpperCase()
    }
    return payout_type;
}
...
function getIsTargeting(field, data) {
    ...
    if ((data['countries'] && data['countries'].length) || data['platform']) {
    ...
    try {
        if (data['countries']) {
            for (let item of data['countries']) {
                geo_targeting['country_allow'].push({ key: item.toUpperCase(), value: item });
            }
        }
    } catch (error) { }
...
function getDeviceTargeting(field, data) {
    ...
    if (data['platform']) {
        ...
        let platform = data['platform'].toLowerCase();
        if (platform == 'android' || platform == 'ios') {
            device_targeting.os.push(platform);
        }
    }
...
function getCreative(field, data) {
    let creative = [];
    if (data['creatives']) {
        for (let item in data['creatives']) {
            for (let ele of data['creatives'][item]) {
                let tempCreative = defaultCreative();
                tempCreative['name'] = item;
                tempCreative['creative_file'] = ele['url'];
                tempCreative['height'] = ele['height'];
                tempCreative['width'] = ele['width'];
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = appSamurai[action](field, offer);
