...
const debug = require("debug")("darwin:plugin:Pentamob");
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.network_id && credentials.api_key && credentials.publisher_id) {
        let network_id = credentials['network_id'];
        let api_key = credentials['api_key'];
        let publisher_id = credentials['publisher_id'];
        let apiBaseurl = "http://" + network_id + "/?s=pub/Offer/api/pub_id/" + publisher_id + "/key/" + api_key + "&page_size=" + apilimit + "&page=";
        // debug(apiBaseurl)
...
function getPreviewUrl(field, data) {
    ...
    if (data.package_name) {
        if (isNaN(data.package_name))
            preview_url = android + data.package_name;
        else
            preview_url = ios + data.package_name;
    }
    return preview_url;
}
...
function getGeoTargeting(field, data) {
    ...
    if (data.geo && data.geo !== []) {
        data.geo.map(objGeo => {
            geo_targeting.country_allow.push({ key: objGeo, value: objGeo });
        })
    }
...
function getDeviceTargeting(field, data) {
    ...
    if (data.device) {
        data.device.map(tempPlt => {
            if (tempPlt == "tablet" || tempPlt == "phone") {
                if (!isMobile) {
                    device_targeting.device.push('mobile');
                }
                isMobile = true;
            }
            else if ((tempPlt == 'desktop' || tempPlt == 'pc')) {
                device_targeting.device.push('desktop');
            }
            else if (tempPlt == "others") {
                device_targeting.device.push('other');
            }
        })
    }
    if (data.platform) {
        if (data.platform == 'ios') {
            ...
            device_targeting.os.push('other');
        }
    }
    if (data.min_os_version && data.platform == 'ios') {
        device_targeting.os_version.push({ os: 'ios', version: data.min_os_version, version_condition: 'gte' });
    }
    if (data.min_os_version && data.platform == 'android') {
        device_targeting.os_version.push({ os: 'android', version: data.min_os_version, version_condition: 'gte' });
    }
    if (data.max_os_version && data.platform == 'ios') {
        device_targeting.os_version.push({ os: 'ios', version: data.max_os_version, version_condition: 'lte' });
    }
    if (data.max_os_version && data.platform == 'android') {
        device_targeting.os_version.push({ os: 'android', version: data.max_os_version, version_condition: 'lte' });
    }
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = pentamob[action](field, offer);
