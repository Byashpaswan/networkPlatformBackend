...
const debug = require("debug")("darwin:Plugin:ricefruit");
const moment = require('moment');
const limit = 500;
exports.countApiPages = (result) => {
    let page = 0;
    try {
        if (result.total_offers_num) {
            page = result.total_offers_num
        }
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.api_token) {
        ...
        let apiBaseurl = `https://api.ricefruit.cn/v1?token=${api_token}&page=${page}&limit=${apilimit}`;
            ...
            do {
                ...
                if (result) {
                    ...
                    if (valid === true) {
                        ...
                        content['domain'] = 'api.ricefruit.cn';
...
exports.checkValid = (result) => {
    if (result &&  result.success && result.offers) {
        return true;
    }
    else {
        return false;
...
function getOfferCapping(field, data) {
    let cap = defaultCap();
     ...
     if(data.daily_click){
        cap.daily_clicks=data.daily_click
     }
    return cap;
}
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = Ricefruit[action](field, offer);
