...
const debug = require("debug")("darwin:Plugin:Artofclick");
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.api_key) {
        ...
        let apiBaseurl = "http://api.artofclick.com/web/Api/v2.3/offer.json?api_key=" + api_key;
        // debug(apiBaseurl)
            ...
            do {
                ...
                if (result) {
                    ...
                    if (valid === true) {
                        ...
                        content['domain'] = "api.artofclick.com";
...
function getCurrency(field, data) {
    let currency = 'USD';
    if (data.currency && data.currency != undefined && data.currency != null) {
...
function getPreviewUrl(field, data) {
    let preview_url = '';
    if (data.previewUrl && data.previewUrl !== undefined)
...
function getRevenueType(field, data) {
    ...
    if (data.payoutType && data.payoutType != undefined && data.payoutType != null) {
...
function getIsCapEnabled(field, data) {
    ...
    if (data.dailyCap !== undefined && data.dailyCap != 0)
        cap_enable = true;
    return cap_enable;
}
...
function getOfferCapping(field, data) {
    let cap = defaultCap();
    if (data.dailyCap !== undefined && data.dailyCap !== 0 && data.dailyCap != null) {
...
function getIsTargeting(field, data) {
    ...
    if ((data.countries !== undefined && data.countries.length) || (data.os !== undefined && data.os.length)) {
...
function getDeviceTargeting(field, data) {
    ...
    if (data.os !== undefined && data.os.length) {
        data.os.map(os => {
            device_targeting.os.push(os.toLowerCase());
        })
    }
    if (data.osVersionMinimum && data.osVersionMinimum != undefined && data.osVersionMinimum != null && data.os.length) {
        data.os.map(os => {
            device_targeting.os_version.push({ os: os.toLowerCase(), version: data.osVersionMinimum, version_condition: 'gte' })
        })
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = Artofclick[action](field, offer);
