...
const debug = require("debug")("darwin:Plugin:gourdmobi");
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.network_id && credentials.api_key) {
        ...
        let apiBaseurl = "http://" + network_id + "/v4/offer/get?code=" + api_key + "&page_size=" + apilimit + "&page=";
...
exports.checkValid = (result) => {
    if (result.success === true && result.offers) {
        //valid credentials gourdmobi  
        return true;
    }
    else {
        return false;
...
function getIsTargeting(field, data) {
    ...
    if ((data.platforms || data.platforms.length) || (data.countries && data.countries.length)) {
...
function getCreative(field, data) {
    ...
    if (data.creatives && data.creatives.length) {
        for (let i of data.creatives) {
            ...
            if (i.pixel && i.hasOwnProperty('pixel')) {
                if (i.pixel.includes("*")) {
                    splitString = i.pixel.split('*');
                } else if (i.pixel.includes("x")) {
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = gourdmobi[action](field, offer);
