...
const debug = require("debug")("darwin:Plugin:funnyStore");
...
exports.countApiPages = (response) => {
    try {
        let nextpage = false;
        if (response.size && response.size == limit) {
            nextpage = true;
        }
        return nextpage;
    }
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.affiliate_id && credentials.api_key) {
        let affiliate_id = credentials['affiliate_id'];
        let api_key = credentials['api_key'];
        // QW5kcjE1NzA4NjY1MzQwMjE
        let apiBaseurl = "http://u.z2z.org/getApp?affid=" + affiliate_id + "&apikey=" + api_key + "&size=" + apilimit + "&page=";
        // debug(apiBaseurl)
...
exports.checkValid = (result) => {
    if (result.result == true && result.info == "OK") {
        return true;
    }
    else {
        return false;
...
function getPreviewUrl(field, data) {
    let preview_url = '';
    if (data.previewLink && data.previewLink !== undefined)
...
function getRevenueType(field, data) {
    ...
    if (data.payType !== undefined && data.payType !== null) {
        revenue_type['offer_type'] = data.payType.toLowerCase();
    }
    return revenue_type;
}
...
function getPayoutType(field, data) {
    ...
    if (data.payType !== undefined && data.payType !== null) {
        payout_type['offer_type'] = data.payType.toLowerCase();
    }
    return payout_type;
}
...
function getIsTargeting(field, data) {
    ...
    if (data.os !== undefined || data.geo !== undefined) {
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = funnyStore[action](field, offer);
