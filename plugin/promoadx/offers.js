...
const debug = require("debug")("darwin:Plugin:Promoadx");
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.api_key && credentials.network_id && credentials.username) {
        let username = credentials['username'];
        let api_key = credentials['api_key'];
        let network_id = credentials['network_id'];
        let apiBaseurl = "https://" + network_id + "/api/affoffer/index?type=all";
        // debug(apiBaseurl)
...
exports.checkValid = (result) => {
    if (result.name === undefined && result.message === undefined && result.code === undefined && Array.isArray(result)) {
...
function getRevenue(field, data) {
    let revenue = 0.0;
    if (data.payout && data.payout.length && data.payout[0].payout)
        revenue = data.payout[0].payout;
    return revenue;
}
...
function getOfferCapping(field, data) {
    let cap = defaultCap();
    if (data.cap) {
        if (data.cap.cap_budget) {
            cap.daily_revenue = data.cap.daily;
        }
        if (data.cap.cap_conversion) {
            cap.daily_conv = data.cap.cap_conversion;
        }
        if (data.cap.cap_click) {
            cap.daily_clicks = data.cap.cap_click;
        }
    }
    return cap;
}
...
function getIsTargeting(field, data) {
    ...
    if ((data.geo && data.geo.length) || (data.platform && data.platform.length)) {
    ...
    if (data.geo && data.geo.length) {
        data.geo.map(obj => {
            if (obj.code && obj.country) {
                geo_targeting.country_allow.push({ key: obj.code, value: obj.country });
            }
        })
    }
...
function getDeviceTargeting(field, data) {
    ...
    if (data.platform && data.platform.length) {
        data.platform.map(obj => {
            if ((obj.platform == 'Mobile' || obj.platform == 'Tablet') && !device_targeting.device.includes('mobile')) {
                device_targeting.device.push('mobile');
            }
            if ((obj.system == 'Android') && !device_targeting.os.includes('android')) {
                device_targeting.os.push('android');
                os = 'android';
            }
            else if (obj.system == 'iOS' && !device_targeting.os.includes('ios')) {
                device_targeting.os.push('ios');
                os = 'ios';
            }
            if (os && obj.version) {
                let version = obj.version.split(os + ":");
                if (version[1]) {
                    version_arr = version[1].split('-');
                    version_arr.map(obj => {
                        device_targeting.os_version.push({ os: os, version: obj, version_condition: 'eq' });
                    });
                }
            }
        });
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = Promoadx[action](field, offer);
