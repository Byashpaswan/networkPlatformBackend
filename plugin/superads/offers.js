...
const debug = require("debug")("darwin:Plugin:Superads");
...
exports.countApiPages = (response) => {
    let page = 0;
    try {
        if (response.offers && response.total_page) {
...
exports.getPid = (url) =>{
    ...
    return urlData.searchParams.get('ch_id');
}
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.api_key && credentials.network_id && credentials.username) {
        ...
        let apiBaseurl = "http://" + network_id + "/v1/jsons?token=" + api_key + "&u=" + username + "&page_size=" + apilimit + "&page=";
        // debug(apiBaseurl)
...
exports.checkValid = (result) => {
    if (result.total_page && result.offers) {
...
function getRevenueType(field, data) {
    ...
    if (data.pricing_mode)
        revenue_type.offer_type = data.pricing_mode;
    return revenue_type;
}
...
function getPayoutType(field, data) {
    ...
    if (data.pricing_mode)
        payout_type.offer_type = data.pricing_mode;
    return payout_type;
}
...
function getGeoTargeting(field, data) {
    ...
    if (data.target_geo) {
        let country = data.target_geo.split(',');
        country.map(obj => {
    ...
    try {
        if (data.creative && data.creative.length) {
            data.creative.map(obj => {
                ...
                if (obj.id) {
                    crt_status = true;
                    tempcreative.creative_id = obj.id;
                }
                if (obj.type) {
                    crt_status = true;
                    if (obj.type == 1)
                        tempcreative.creative_type = 'banner';
                    if (obj.type == 2)
                        tempcreative.creative_type = 'video';
                }
                if (obj.link) {
                    crt_status = true;
                    tempcreative.creative_file = obj.link;
                }
                if (crt_status) {
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = Superads[action](field, offer);
