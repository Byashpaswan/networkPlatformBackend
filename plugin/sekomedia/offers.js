...
const debug = require("debug")("darwin:Plugin:sekomedia");
...
exports.countApiPages = (response) => {
    let nextPage = false;
    try {
        if (response.success && response.offers && response.error_messages) {
            nextPage = true;
        }
        // debug(nextPage)
        return nextPage;
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.api_key) {
        ...
        let apiBaseurl = "https://api.sekomedia.com/feed?key=" + api_key + "&kpi=true&limit=" + apilimit + "&skip=";
        // debug(apiBaseurl,'----')
            ...
            do {
                ...
                if (result) {
                    ...
                    if (valid === true) {
                        ...
                        content['domain'] = "api.sekomedia.com";
...
exports.checkValid = (result) => {
    if (result.success && result.error_messages && result.offers) {
...
function getIsTargeting(field, data) {
    ...
    if (data.Platforms || data.Countries) {
...
function getDeviceTargeting(field, data) {
    ...
    if (data.Platforms) {
        let plt = data.Platforms.split(',');
        plt.map(obj => {
            if ((obj == 'iPhone' || obj == 'iPad') && !device_targeting.os.includes('ios')) {
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = sekomedia[action](field, offer);
