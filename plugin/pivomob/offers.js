...
const debug = require("debug")("darwin:Plugin:pivomob");
...
exports.apiCall = async (credentials, page, apilimit) => {
  if (credentials.publisher_id && credentials.access_token) {
    let publisher_id = credentials["publisher_id"];
    let access_token = credentials["access_token"];
    let apiBaseurl =
      "http://api.campaign.pivomob.com/get_campaigns?publisher_id=" + publisher_id + "&access_token=" + access_token
      ...
      do {
        ...
        if (result) {
          ...
          if (valid === true) {
            ...
            content['domain'] = "api.campaign.pivomob.com";
...
exports.checkValid = result => {
  if (result && result.status == 200 && result.campaigns && result.campaigns.length) {
    return true;
  } else {
    return false;
  }
...
function getIsTargeting(field, data) {
  ...
  if (data.Platform || data.min_os_version || (data.Geo && data.Geo.length)) {
...
function formateOffers(offer) {
  ...
  array.map(function (obj) {
    try {
      ...
      let data = pivomob[action](field, offer);
