...
const debug = require("debug")("darwin:Plugin:aduclick");
...
exports.countApiPages = (result) => {
  let page = 0;
  if (result.data) {
    page = Math.ceil(result.data.pages);
  }
  return page;
}
...
exports.apiCall = async (credentials, page, apilimit) => {
  if (credentials.Username && credentials.Password) {
    ...
    let apiBaseurl = 'https://panel.aduclick.io/api/v1/affiliate/offers?limit=' + apilimit + '&page=';
      ...
      do {
        ...
        if (result) {
          ...
          if (valid === true) {
            ...
            content['domain'] = "panel.aduclick.io";
...
exports.checkValid = (result) => {
  if (result && result.success == true && result.data && result.data.items && result.data.items.length) {
    return true;
  } else {
    return false;
  }
...
function getIsTargeting(field, data) {
  ...
  if (data.targeting.countries.values && data.targeting.countries.values.length) {
...
function getGeoTargeting(field, data) {
  ...
  if (data.targeting.countries.values && data.targeting.countries.values.length) {
    for (let obj of data.targeting.countries.values) {
...
function formateOffers(offer) {
  ...
  array.map(function (obj) {
    try {
      ...
      let data = aduclick[action](field, offer);
