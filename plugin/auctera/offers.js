...
const debug = require("debug")("darwin:Plugin:Auctera");
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.network_id && credentials.token) {
        ...
        let apiBaseurl = `https://${network_id}/offer?token=${token}&&per-page=${apilimit}&page=`;
...
exports.checkValid = (result) => {
    if (typeof result == "object" && result.items && result.meta) {
        //valid credentials Auctera  
        return true;
    }
    else {
        return false;
...
function getIsCapEnabled(field, data) {
    ...
    if (data.publisher_daily_cap || data.publisher_total_cap) {
        cap_enable = true;
    }
    return cap_enable;
}
...
function getOfferCapping(field, data) {
    let cap = defaultCap();
    if (data.publisher_daily_cap) {
        cap.daily_conv = data.publisher_daily_cap;
    }
    if (data.publisher_total_cap) {
...
function getDeviceTargeting(field, data) {
    ...
    if (data.os && data.os.length) {
        data.os.map(obj => {
            if (obj == "android") { device_targeting.os.push(obj.toLowerCase()); }
            if (obj == "ios") { device_targeting.os.push(obj.toLowerCase()); }
        })
    }
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = Auctera[action](field, offer);
