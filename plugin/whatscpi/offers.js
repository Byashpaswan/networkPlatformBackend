...
const debug = require("debug")("darwin:Plugin:Whatscpi");
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.api_key && credentials.app_id) {
        let app_id = credentials['app_id'];
        let api_key = credentials['api_key'];
        let apiBaseurl = "http://api.whatscpi.com/index.php?app_key=" + api_key + "&p=getoffer&m=advert&app_id=" + app_id + "&pagesize=" + apilimit + "&page=";
        // debug(apiBaseurl)
            ...
            do {
                ...
                if (result) {
                    ...
                    if (valid === true) {
                        ...
                        content['domain'] = "api.whatscpi.com";
...
function getTrackingLink(field, data) {
    let tracking_link = '';
    if (data.click_url)
        tracking_link = data.click_url.split('&aff_sub={aff_sub}')[0];
...
function getDeviceTargeting(field, data) {
    ...
    let os = '';
    if (data.platform) {
        let device_plt = data.platform.split(',');
        device_plt.map(obj => {
    ...
    if (data.minOS && os) {
        let version_arr = data.minOS;
        if (isNaN(version_arr)) {

        }
        else {
            device_targeting.os_version.push({ os: os, version: data.minOS, version_condition: 'gte' });
        }
    }
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = whatsCpi[action](field, offer);
