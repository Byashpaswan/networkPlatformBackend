...
const debug = require("debug")("darwin:Plugin:sixpush");
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.api_key && credentials.publisher_id) {
        ...
        let apiBaseurl = "http://offers.sixpush.com/" + publisher_id + "/" + api_key;
        // debug(apiBaseurl,'----')
            ...
            do {
                ...
                if (result) {
                    ...
                    if (valid === true) {
                        ...
                        content['domain'] = "offers.sixpush.com";
...
function getCategory(field, data) {
    ...
    if (data.categories && data.categories.length) {
        data.categories.map((cat) => { category.push(cat) })
    }
    return category;
}
...
function getRevenueType(field, data) {
    ...
    if (data.is_cpi && data.is_cpi == true) {
...
function getGeoTargeting(field, data) {
    ...
    if (data.targeting && data.targeting.length) {
        data.targeting.map(obj => {
            if (obj.country && obj.country['allow'] && obj.country['allow'].length) {
                obj.country['allow'].map((country) => {
...
function getDeviceTargeting(field, data) {
    ...
    if (data.targeting && data.targeting.length) {
        data.targeting.map((obj) => {
            if (obj.os && obj.os['allow'] && obj.os['allow'].length) {
                obj.os['allow'].map((obj) => {
                    if (obj.name == 'Android' || obj.name == 'iOS') {
                        device_targeting.os.push((obj.name.toLowerCase()));
                        if (obj.version) {
                            device_targeting.os_version.push({ os: obj.name.toLowerCase(), version: obj.version.replace(">", "").replace("<", ""), version_condition: obj.comparison.toLowerCase() });
                        }
                    }
                })
            }
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = sixpush[action](field, offer);
