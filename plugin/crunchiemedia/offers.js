...
const debug = require("debug")("darwin:Plugin:Crunchiemedia");
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.api_key) {
        ...
        let apiBaseurl = "https://api.crunchiemedia.com/api/index.cfm?token=" + api_key;
            ...
            do {
                ...
                if (result) {
                    ...
                    if (valid === true) {
                        ...
                        content['domain'] = "api.crunchiemedia.com";
...
exports.checkValid = (result) => {
    if (result.Error.Code == 0 && result.Error.Message == "") {
        //valid credentials Crunchiemedia  
        return true;
    }
    else {
        return false;
...
function getTrackingLink(field, data) {
    ...
    if (data.TrackingLink) {
        link = data.TrackingLink.split('?');
        if (link.length > 1) {
            parameter = link[1].split('&');
            parameter.map(obj => {
                splitParm = obj.split('=');
                Parms[splitParm[0]] = splitParm[1];
            })
        }
        tracking_link = link[0] + "?o=" + Parms.o + "&p=" + Parms.p;
    }
...
function getIsTargeting(field, data) {
    ...
    if (data.SupportedCountries.length && data.SupportedPlatforms.length) {
...
function getDeviceTargeting(field, data) {
    ...
    if (data.SupportedPlatforms.length) {
        data.SupportedPlatforms.map(os => {
...
function getCreative(field, data) {
    ...
    if (data.Creatives.length) {
        for (let i of data.Creatives) {
            ...
            createobj.creative_type = i.CreativeType;
            createobj.height = i.Dimensions.Height;
            createobj.width = i.Dimensions.Width;
            createobj.creative_id = i.CreativeID;
            createobj.creative_file = i.CreativeURL;
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = Crunchiemedia[action](field, offer);
