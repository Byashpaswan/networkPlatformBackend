...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.api_key && credentials.affiliate_id) {
        let api_key = credentials['api_key'];
        let affiliate_id = credentials['affiliate_id'];
        let apiBaseurl = `http://s2s.moca-tech.net/v2.0/sync?aff_id=${affiliate_id}&apikey=${api_key}`;
            ...
            do {
                ...
                if (result) {
                    ...
                    if (valid === true) {
                        ...
                        content['domain'] = "s2s.moca-tech.net";
...
exports.checkValid = (result) => {
    if (result.result && result.info == "OK" && result.list) {
        //valid credentials mocaTech  
        return true;
    }
    else {
        return false;
...
function getRevenueType(field, data) {
    ...
    if (data.paymodel) {
        revenue_type.offer_type = data.paymodel.toLowerCase();
    }
    return revenue_type;
}
...
function getPayoutType(field, data) {
    ...
    if (data.paymodel) {
        payout_type.offer_type = data.paymodel.toLowerCase();
    }
    return payout_type;
}
...
function getOfferCapping(field, data) {
    let cap = defaultCap();
    if (data.daily_caps) {
        cap.daily_conv = data.daily_caps;
    }
    return cap;
}
...
function getIsTargeting(field, data) {
    ...
    if ((data.platform) || (data.countries) || (data.device)) {
...
function getDeviceTargeting(field, data) {
    ...
    if (data.platform) {
        if (data.platform.includes("ANDROID")) { device_targeting.os.push("android"); }
        else if (data.platform.includes("IOS")) { device_targeting.os.push("ios"); }
    }
    if (data.device) {
        let device = data.device.split('|');
        device.map(obj => {
            if (obj == "MOBILE") { device_targeting.device.push("mobile"); }
            else if (obj == "PC") { device_targeting.device.push("desktop"); }
        })
    }
...
function getCreative(field, data) {
    ...
    if (data.creative) {
        ...
        temp.creative_file = data.creative;
        creative.push(temp);
    }
    return creative;
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = mocaTech[action](field, offer);
