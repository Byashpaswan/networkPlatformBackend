...
const debug = require("debug")("darwin:Plugin:imxbidding");
...
exports.countApiPages = (response) => {
    let page = 0;
    try {
        if (response.next_page && response.total_pages && response.message == 'success') {
...
exports.apiCall = async (credentials, page = 0 , apilimit = 0 ) => {
    // console.log(" api credentials -> " , credentials );
    if ( credentials.token ) {
        // let network_id = credentials['network_id'].trim();
        let token = credentials['token'].trim();
        // http://api.imxbidding.net/v2/offer/pull?token=1795220c9dae4e9bb6dbcd20f52b6cd4

        let apiBaseurl = "http://api.imxbidding.net/v2/offer/pull?token=" + token  ///   + "&size=" + apilimit + "&page=";
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            do {
                ...
                // console.log(" result of api call -> " , result  , " result of api call ");
                if (result) {
                    valid = this.checkValid(result.data)
                    if (valid === true) {
                        let data = this.fetchResponse(result.data);
                        content['domain'] = content.credentials['network_id']|| 'api.imxbidding.net';
...
exports.checkValid = (result) => {
    if (result.data && result.code === 0 && result.message == 'success') {
...
function getCategory(field, data) {
    ...
    if (data.app_category) {
        category = data.app_category.split(',');
    }
    return category;
}
...
function getThumbnail(field, data) {
    let thumbnail = '';
    if (data.icons && data.icons.length && data.icons[0].url) {
...
function getIsCapEnabled(field, data) {
    ...
    if (data.cap_daily || data.click_cap_daily || data.r_cap_daily) {
        cap_enable = true;
    }
    return cap_enable;
}
...
function getOfferCapping(field, data) {
    let cap = defaultCap();
    if (data.cap_daily) {
        cap.daily_conv = data.cap_daily;
    }
    if (data.click_cap_daily) {
        cap.daily_clicks = data.click_cap_daily;
    }
    return cap;
}
...
function getIsTargeting(field, data) {
    ...
    if ((data.countries && data.countries.length) || data.os) {
...
function getDeviceTargeting(field, data) {
    ...
    if (data.os) {
        if (data.os == 'ANDROID') {
            ...
            if (data.app_min_ver) {
                device_targeting.os_version.push({ os: 'android', version: data.app_min_ver, version_condition: 'gte' });
            }
        }
        if (data.os == 'IOS') {
            device_targeting.os.push('ios');
            if (data.app_min_ver) {
                device_targeting.os_version.push({ os: 'ios', version: data.app_min_ver, version_condition: 'gte' });
            }
        }
    }
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = imxbidding[action](field, offer);
