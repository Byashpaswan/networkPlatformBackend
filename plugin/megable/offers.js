...
const debug = require("debug")("darwin:Plugin:Mobxmedia");
...
exports.countApiPages = (response) => {
    ...
    try {
        if (response.total && response.pagesize) {
            page = Math.ceil(response.total / response.pagesize);
        }
        return page;
    }
    catch (err) {
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.network_id && credentials.api_key && credentials.app_id) {
        ...
        let apiBaseurl = "http://" + network_id + "/?request_uri=/offer/Getoffer/getOffer&app_id=" + app_id + "&app_key=" + api_key + "&pagesize=" + apilimit + "&page=";
        // debug(apiBaseurl)
...
function getIsTargeting(field, data) {
    ...
    if (data.platform || data.countries.length) {
...
function getDeviceTargeting(field, data) {
    ...
    if (data.minOS && data.platform) {
        device_targeting.os_version.push({ os: data.platform, version: data.minOS, version_condition: 'gte' })
    }
...
function getCreative(field, data) {
    let creative = [];
    if (data.images) {
        keys = Object.keys(data.images);
        for (let i of keys) {
            createObj = defaultCap();
            splitString = i.split("*");
            createObj.height = splitString[0];
            createObj.width = splitString[1];
            createObj.creative_file = data.images[i];
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = Mobxmedia[action](field, offer);
