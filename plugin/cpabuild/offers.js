...
const debug = require("debug")("darwin:Plugin:Cpabuild");
...
exports.apiCall = async (credentials, page, limit) => {
    try {
        if (credentials.network_id && credentials.api_key && credentials.user_id) {
            let apiBaseurl = `https://${credentials.network_id}/public/offers/feed.php?user_id=${credentials.user_id}&api_key=${credentials.api_key}`
...
function getDescription(field, data) {
    return data.anchor ? data.anchor : '';
}

function getKpi(field, data) {
    return data.conversion ? data.conversion : "";
}
...
function getRevenue(field, data) {
    return data.payout ? parseFloat(data.payout) : (data.user_payout ? parseFloat(data.user_payout) : 0.0)
}
...
function getStatusLabel(field, data) {
    return data.url ? "active" : "no_link"
}
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = Cpabuild[action](field, offer);
