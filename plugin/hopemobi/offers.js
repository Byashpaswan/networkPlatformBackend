...
const debug = require("debug")("darwin:Plugin:hopemobi");
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.token) {
        ...
        let apiBaseurl = "http://api.hopemobi.net/v2/offer/pull?token=" + token;
            ...
            do {
                ...
                if (result) {
                    ...
                    if (valid === true) {
                        ...
                        content['domain'] = "api.hopemobi.net";
...
exports.checkValid = (result) => {
    if (result && result['message'] == 'success' && result['data'].length) {
        return true;
    }
    else {
        return false;
...
function getCategory(field, data) {
    let category = [];
    if (data['app_category'] && data['app_category'].trim()) {
        category.push(data['app_category'].trim().toLowerCase());
    }
    return category;
}
...
function getIsCapEnabled(field, data) {
    ...
    if ((data['cap_daily'] && data['cap_daily'] > 0) || (data['click_cap_daily'] && data['click_cap_daily'] > 0)) {
        cap_enable = true;
    }
    return cap_enable;
}
...
function getOfferCapping(field, data) {
    let cap = defaultCap();
    if (data['cap_daily'] && data['cap_daily'] > 0) {
        cap.daily_conv = parseInt(data['cap_daily']);
    }
    if (data['click_cap_daily'] && data['click_cap_daily'] > 0) {
        cap.daily_clicks = parseInt(data['click_cap_daily']);
    }
    return cap;
}
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = hopemobi[action](field, offer);
