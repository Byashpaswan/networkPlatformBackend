...
const debug = require("debug")("darwin:Plugin:VNative");
...
exports.countApiPages = (response) => {
    let page = 0;
    try {
        if (response.campaigns && response.totalPage && response.totalActiveOffers) {
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.api_key && credentials.password && credentials.user_id) {
        let user_id = credentials['user_id'];
        let password = credentials['password'];
        let api_key = credentials['api_key'];
        let apiBaseurl = "https://admedx.trackzgo.com/tracking/api/sync/" + api_key + "?userID=" + user_id + "&password=" + password + "&limit=" + apilimit + "&page=";
        // debug(apiBaseurl)
            ...
            do {
                ...
                if (result) {
                    ...
                    if (valid === true) {
                        ...
                        content['domain'] = "admedx.trackzgo.com";
...
exports.checkValid = (result) => {
    if (result && result.campaigns && result.campaigns.offer) {
...
function getCategory(field, data) {
    ...
    if (data.offerCategory) {
        category = data.offerCategory.split(',');
    }
    return category;
}
...
function getRevenueType(field, data) {
    ...
    if (data.conversionType)
        revenue_type.offer_type = data.conversionType;
    return revenue_type;
}
...
function getPayoutType(field, data) {
    ...
    if (data.conversionType)
        payout_type.offer_type = data.conversionType;
    return payout_type;
}
...
function getGeoTargeting(field, data) {
    ...
    if (data.country && data.country != 'ALL') {
        let country = data.country.split(',');
        country.map(obj => {
            if (obj && obj != 'ALL') {
...
function getDeviceTargeting(field, data) {
    ...
    if (data.os) {
        ...
        os.map(obj => {
            ...
            else if (obj == 'Windows mobile' && !device_targeting.os.includes('windows')) {
                device_targeting.os.push('windows');
            }

        })
    }
    if (data.osVersion && data.osVersion != 'All') {
        let version_arr = data.osVersion.split(',');
        version_arr.map(version => {
            let obj = version.split(' ');
            if (obj[0] && obj[1] && !isNaN(obj[1])) {
                if (obj[0] == 'ios')
                    device_targeting.os_version.push({ os: 'ios', version: obj[1], version_condition: 'gte' });
                if (obj[0] == 'android')
                    device_targeting.os_version.push({ os: 'android', version: obj[1], version_condition: 'gte' });
            }
        })
    }
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = vnative[action](field, offer);
