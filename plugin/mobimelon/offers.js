...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.api_key && credentials.network_id && credentials.user_id) {
        let api_key = credentials['api_key'];
        let user_id = credentials['user_id'];
        let network_id = credentials['network_id'];
        let apiBaseurl = "http://" + network_id + "/publisherapi/offers/?uid=" + user_id + "&key=" + api_key + "&limit=" + apilimit + "&page=";
        // debug(apiBaseurl)
...
exports.checkValid = (result) => {
    if (result.status && result.status == 1 && result.data) {
...
function getRevenue(field, data) {
    let revenue = 0.0;
    if (data.payout && data.payout != 'Variable' && !isNaN(data.payout))
        revenue = data.payout;
    return revenue;
}
...
function getRevenueType(field, data) {
    ...
    if (data.paytype && data.paytype.length)
        revenue_type.offer_type = data.paytype[0];
    return revenue_type;
}
...
function getPayoutType(field, data) {
    ...
    if (data.paytype && data.paytype.length)
        payout_type.offer_type = data.paytype[0];
    return payout_type;
}
...
function getOfferCapping(field, data) {
    let cap = defaultCap();
    if (data.caps) {
        if (data.caps.type && data.caps.type == 'Daily') {
            if (data.caps.amount) {
                cap.daily_conv = data.caps.amount;
            }
        }
    }
    return cap;
...
function getIsTargeting(field, data) {
    ...
    if (data.country || data.device || data.platform) {
