...
const debug = require("debug")("darwin:Plugin:Mobobeat");
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.api_key && credentials.network_id) {
        ...
        let apiBaseurl = "https://" + network_id + "/publisherAPI/getCampaigns/" + api_key + "?version=v3";
        // debug(apiBaseurl );
...
exports.checkValid = (result) => {
    if (result.campaigns && result.status == "OK") {
...
function getIsTargeting(field, data) {
    ...
    if ((data.oss && data.oss.length) || (data.devicetypes && data.devicetypes.length) || (data.include_geos && data.include_geos.length)) {
...
function getGeoTargeting(field, data) {
    ...
    if (data.include_geos && data.include_geos.length) {
        data.include_geos.map(obj => {
            if (obj)
...
function getDeviceTargeting(field, data) {
    ...
    if (data.devicetypes && data.devicetypes.length) {
        data.devicetypes.map(obj => {
            if ((obj == 'Smartphone' || obj == 'Tablet') && !device_targeting.device.includes('mobile')) {
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = Mobobeat[action](field, offer);
