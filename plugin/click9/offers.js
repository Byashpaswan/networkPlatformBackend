...
const debug = require("debug")("darwin:Plugin:Clicks9");
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.network_id && credentials.api_key) {
        ...
        let apiBaseurl = `http://${network_id}/api/?apikey=${api_key}`;
...
function getTrackingLink(field, data) {
    let tracking_link = '';
    if (data.TrackingUrl) {
        tracking_link = data.TrackingUrl.includes('&clickid') ? data.TrackingUrl.split('&clickid')[0] : data.TrackingUrl;
    }
...
function getEndDate(field, data) {
    ...
    try {
        if (data.ExpiryDate) {
            end_date = moment(data.ExpiryDate, 'YYYY/MM/DD');
...
function getRevenueType(field, data) {
    ...
    if (data.PayoutModel) {
        revenue_type.offer_type = data.PayoutModel.toLowerCase();
    }
    return revenue_type;
}
...
function getPayoutType(field, data) {
    ...
    if (data.PayoutModel) {
        payout_type.offer_type = data.PayoutModel.toLowerCase();
    }
    return payout_type;
}
...
function getIsTargeting(field, data) {
    ...
    if ((data.OS && data.OS.length && !data.OS.includes('All')) || (data.Countries && data.Countries.length && !data.Countries.includes('All'))) {
...
function getGeoTargeting(field, data) {
    ...
    if (data.Countries && data.Countries.length) {
        data.Countries.map(obj => {
            ...
            let i = config.country.findIndex(ele => ele.key === obj.toUpperCase());
            if (i > -1) {
                geo_targeting.country_allow.push({ key: config.country[i].key, value: config.country[i].value })
            }
            else {
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = Clicks9[action](field, offer);
