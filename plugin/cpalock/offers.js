...
const debug = require("debug")("darwin:Plugin:Cpalock");
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.api_key && credentials.publisher_id) {
        let api_key = credentials['api_key'];
        let publisher_id = credentials['publisher_id'];
        let apiBaseurl = "https://www.cpalock.com/offer_feed.php?pubid=" + publisher_id + "&api_key=" + api_key + "&limit=" + apilimit;
        // debug(apiBaseurl)
            ...
            do {
                ...
                if (result) {
                    ...
                    if (valid === true) {
                        ...
                        content['domain'] = "www.cpalock.com";
...
function getDeviceTargeting(field, data) {
    ...
    try {
        if (data.platform) {
            ...
            platform.map(obj => {
                obj = obj.trim();
                let os_platform = obj.split(' ');
                let os = '';
                if (os_platform[0]) {
                    os_platform[0] = os_platform[0].trim();
                    if ((os_platform[0] == 'Desktop') && !device_targeting.device.includes('desktop')) {
                        device_targeting.device.push('desktop');
                    }
                    else if ((os_platform[0] == 'Android') && !device_targeting.os.includes('android')) {
                    ...
                    else if ((os_platform[0] == 'Apple iOS' || os_platform[0] == 'iOS' || os_platform[0] == 'iPad' || os_platform[0] == 'iPhone') && !device_targeting.os.includes('ios')) {
                    ...
                    else if (os_platform[0] == 'Microsoft Windows OS' && !device_targeting.os.includes('windows')) {
                        device_targeting.os.push('windows');
                        os = 'windows';
                    }
                    else if (os_platform[0] == 'Blackberry RIM OS' && !device_targeting.os.includes('blackberry')) {
                        device_targeting.os.push('blackberry');
                        os = 'blackberry';
                    }
                }
                if (os_platform[1] && os) {
                    os_platform[1] = os_platform[1].trim();
                    if (isNaN(os_platform[1])) {
                        if (os_platform[1].includes('+')) {
                            let version = os_platform[1].split('+');
                            if (version[0] && !isNaN(version[0])) {
                                version[0] = version[0].trim();
                                device_targeting.os_version.push({ os: os, version: version[0], version_condition: 'gte' });
                            }
                        }
                        if (os_platform[1].includes('-')) {
                            let version = os_platform[1].split('-');
                            if (version[0] && !isNaN(version[0])) {
                                version[0] = version[0].trim();
                                device_targeting.os_version.push({ os: os, version: version[0], version_condition: 'lte' });
                            }
                        }
                    }
                    else {
                        device_targeting.os_version.push({ os: os, version: os_platform[1], version_condition: 'eq' });
                    }
                }
            })
        }
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = Cpalock[action](field, offer);
