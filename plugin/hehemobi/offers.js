...
const debug = require("debug")("darwin:Plugin:hehemobi");
...
exports.countApiPages = (result) => {
  let page = 0;
  if (result.data) {
    page = Math.ceil(result.total_pages);
  }
  return page;
}
...
exports.apiCall = async (credentials, page, apilimit) => {
  if (credentials.token) {
    ...
    let apiBaseurl = 'http://api.hehemobi.com/v2/offer/pull?token=' + token;
      ...
      do {
        ...
        if (result) {
          ...
          if (valid === true) {
            ...
            content['domain'] = "api.hehemobi.com";
...
function formateOffers(offer) {
  ...
  array.map(function (obj) {
    try {
      ...
      let data = hehemobi[action](field, offer);
