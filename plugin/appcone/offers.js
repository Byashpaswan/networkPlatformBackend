...
const debug = require("debug")("darwin:Plugin:Appcone");
const ENV_OFFERS_LIMIT = process.env.ENV_OFFERS_LIMIT;
const moment = require('moment');
const limit = 1000;
exports.countApiPages = (result) => {
    let page = 0;
    if (result && result.data && result.data.length && result.data.size && result.data.size == limit) {
        page++;
    }
    return page;
}
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.api_key && credentials.network_id) {
        let apiBaseurl = `http://${credentials.network_id}/exchange?auth=${credentials.api_key}&page=${page}&size=${apilimit}&timestamp=${new Date().getTime()}`
        console.log("file: offers.js ~ line 18 ~ exports.apiCall= ~ apiBaseurl", apiBaseurl)
        let result = await makeRequest({
            method: 'get',
            url: apiBaseurl
        });
        // console.log("file: offers.js ~ line 19 ~ exports.apiCall= ~ result", result.data);
        return result;
    }
    else {
        return null;
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = appcone[action](field, offer);
