...
const debug = require("debug")("darwin:Plugin:mobivisits");
...
exports.countApiPages = (response) => {
    let page = 0;
    try {
        if (response.meta && response.meta.pageCount) {
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.network_id && credentials.api_key) {
        ...
        let apiBaseurl = "https://" + network_id + "/offer?token=" + api_key + "&per-page=" + apilimit + "&page=";
...
exports.checkValid = (result) => {
    if (result.items && result.code == undefined) {
        //valid credentials mobivisits  
        return true;
    }
    else {
        return false;
...
function getIsGoalEnabled(field, data) {
    ...
    if (data.goal_id && data.goal_id != null && data.goal_id != undefined) {
        goal_enable = true;
    }
    return goal_enable;
}
...
function getGoals(field, data) {
    let goal = [];
    if (data.goal_id && data.goal_id != null && data.goal_id != undefined) {
        let createGoalObj = defaultGoal();
        createGoalObj.goal_id = data.goal_id;
        goal.push(createGoalObj);
    }
    return goal;
}
...
function getDescription(field, data) {
    let description = '';
    if (data.descr && data.descr !== undefined && data.descr !== null)
...
function getPreviewUrl(field, data) {
    let preview_url = '';
    if (data.url_preview && data.url_preview !== undefined)
        preview_url = data.url_preview;
    return preview_url;
}
...
function getIsCapEnabled(field, data) {
    ...
    if ((data.publisher_total_cap !== undefined && data.publisher_total_cap != 0) || (data.publisher_daily_cap !== undefined && data.publisher_daily_cap !== 0))
        cap_enable = true;
    return cap_enable;
}
...
function getOfferCapping(field, data) {
    let cap = defaultCap();
    if (data.publisher_total_cap && data.publisher_total_cap !== undefined && data.publisher_total_cap != 0) {
        cap.overall_conv = data.publisher_total_cap
    }
    if (data.publisher_daily_cap && data.publisher_daily_cap !== undefined && data.publisher_daily_cap !== 0) {
        cap.daily_conv = data.publisher_daily_cap
    }
    return cap;
}
...
function getIsTargeting(field, data) {
    ...
    if ((data.os && data.os.length) || (data.geo && data.geo.length)) {
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = mobivisits[action](field, offer);
