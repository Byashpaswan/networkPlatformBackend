...
const debug = require("debug")("darwin:Plugin:appleads");
...
exports.countApiPages = (result) => {
    let page = 0;
    try {
        if (result && result['message'] == "Success" && result.totalData) {
            page = result.totalData
        }
        return page;
    }
    catch (err) {
...
exports.apiCall = async (credentials, page, apilimit) => {
    if (credentials.api_key) {
        ...
        let apiBaseurl = 'http://api.appleadstech.com/v3/api_v3?key=' + api_key;
            ...
            do {
                ...
                if (result) {
                    ...
                    if (valid === true) {
                        ...
                        content['domain'] = "api.appleadstech.com";
...
exports.checkValid = (result) => {
    if (result && result.message == "Success" && result.data && result.data.length) {
...
function getIsTargeting(field, data) {
    ...
    if (data.device != "" || data.os != "" || data.countries != "") {
...
function getDeviceTargeting(field, data) {
    ...
    if (data.os) {
        ...
        if (OS == 'android' || OS == 'ios' || OS == 'windows') {
            ...
            if (data.min_osv) {
                device_targeting.os_version.push({ os: OS, version: data.min_osv, version_condition: 'gte' })
            }
        }
    }
...
function formateOffers(offer) {
    ...
    array.map(function (obj) {
        try {
            ...
            let data = appleads[action](field, offer);
