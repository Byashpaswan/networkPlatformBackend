...
const startDate = process.env.SCRAPING_DATE_START ? moment(process.env.SCRAPING_DATE_START).toDate() : moment().startOf('days').subtract(100000, 'days').toDate();
const endDate = process.env.SCRAPING_DATE_END ? moment(process.env.SCRAPING_DATE_END).toDate() : moment().startOf('days').toDate();
...
async function updateAppIds(app_ids) {
    let finalArray = app_ids.map(function (obj) { return mongooseObjectId(obj._id); });
    try {
            ...
            for (let doc = await cursor.next(); doc != null; doc = await cursor.next()) {
                if (app_ids.some(ele => ele.app_id === doc.app_id)) continue;
                    ...
                    if (doc.geo_targeting && doc.geo_targeting.country_allow && doc.geo_targeting.country_allow.length) {
                        ...
                        country = tempObjCountry['key']?.toLowerCase();
                    }
...
exports.startSaveAppIdsScript = async () => {
    try {
        await insertAppIds();
        await publishAppIds();
