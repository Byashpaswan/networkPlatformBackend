...
exports.publish_Content = async (isMultipleContent = false, queue_name, content, persistent, durable, priority = 0, requeque = false, ttl = 0) => {
    ...
    if (pubChannel) {
        if (isMultipleContent && Array.isArray(content)) {
            for (let i = 0; i < content.length; i++) {
                pubAck = await this.publish(queue_name, content[i], persistent, durable, priority);
            }
        }
        else {
            pubAck = await this.publish(queue_name, content, persistent, durable, priority);
        }
    }
    return await Promise.resolve(pubAck);
}

exports.publish = async (queue, content, persistentValue, durableValue, priority) => {
    // console.log("==================Worker Job Publish Function Start");
    return new promise((resolve, reject) => {
        pubChannel.assertQueue(queue, { persistent: persistentValue, durable: durableValue, maxPriority: 20 }).then(res => {
